                ...
                <Link to='/'><Logo/></Link>
                    ...
                    <ul className={` ${show ? 'show' : 'hide'}   flex gap-[22px] max-lg:absolute  max-lg:bg-gray-400 max-lg:w-full max-lg:h-screen max-lg:top-0 duration-300 ease-linear z-10 max-lg:items-center max-lg:justify-center max-lg:flex-col `}>    
                        {["Home",  "Collection", "About" , "Contact"].map((navlink,i) =>(
                            <li  key={i} className='text-[#2A2A2A] font-outfit font-medium leading-[100%] text-[16px]'><NavLink to='/home' className={({isActive}) => ` ${isActive ? ' after:absolute after:w-full after:bottom-[-4px] after:right-0 after:h-[2px] after:bg-[#303030] after:rounded-full' :''} relative after:duration-300 after:ease-linear` }>{navlink.toUpperCase()}</NavLink></li>
                        ))}                             
                    </ul>
